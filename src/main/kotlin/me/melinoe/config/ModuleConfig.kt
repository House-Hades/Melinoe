...
class ModuleConfig internal constructor(file: File) {
    ...
    /**
     * Main constructor for Addons. (config/melinoe/addons/{fileName})
     */
    constructor(fileName: String) : this(File(Melinoe.configFile, "addons/$fileName"))
    ...
    private val file: File = file.apply {
        try {
            ...
            Melinoe.logger.error("Error initializing module config", e)
        }
    }
    ...
    fun save() {
        try {
            ...
            Melinoe.logger.error("Error saving module config.", e)
        }
    }
